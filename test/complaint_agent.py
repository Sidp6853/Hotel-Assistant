...
def analysis_agent(state: ComplaintState) -> ComplaintState:
    ...
    prompt = f"""
You are a hotel complaint analysis system.

Analyze the complaint and return ONLY valid JSON (no markdown, no explanations).

JSON Schema:
{{
  "severity_level": "low | medium | high | critical",
  "category": "string (e.g., room cleanliness, service, noise)",
...
Severity Guidelines:
- critical: Health/safety emergencies, major incidents
- high: Significant service failures, safety concerns
- medium: Room issues, service delays
- low: Minor inconveniences, requests
...
def action_planning_agent(state: ComplaintState) -> ComplaintState:
    ...
    prompt = f"""
You are a hotel operations planner.

Based on the analysis, create internal actions.
...
{{
  "internal_actions": [
    {{
      ...
      "responsible": "role name (e.g., Housekeeping Manager)",
      "priority": "immediate | high | normal | low"
    }}
  ],
  "assigned_department": "department name (e.g., Housekeeping, Front Desk, Maintenance)",
  "estimated_resolution_time": "time estimate (e.g., 30 minutes, 2 hours)"
}}
